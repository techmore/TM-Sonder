...
nonisolated final class SonderHTTPServer: @unchecked Sendable {
    ...
    private func route(connection: NWConnection, request: HTTPRequest, isLocalPeer: Bool) {
        ...
        case let path where path.hasPrefix("/subtitles/"):
            sendSubtitle(connection: connection, path: path)
        default:
    ...
    private func sendLibrary(_ connection: NWConnection, ifNoneMatch: String?) {
        ...
        Task { @MainActor in
            let snapshot = self.library.libraryResponseSnapshot(theme: self.themeSnapshot)
            Task.detached {
    ...
    private func sendStatus(_ connection: NWConnection) {
        Task { @MainActor in
            let snapshot = self.library.statusSnapshot()
            Task.detached {
                let encoder = JSONEncoder()
                encoder.dateEncodingStrategy = .iso8601
                let encoded = (try? encoder.encode(snapshot)) ?? Data("{}".utf8)
    ...
    private func sendAudiobookDetail(_ connection: NWConnection, path: String) {
        guard let id = SonderHTTPRouteID.uuid(from: path) else {
    ...
    // MARK: - Subtitle endpoint

    private func sendSubtitle(connection: NWConnection, path: String) {
        let components = path.split(separator: "/").map(String.init)
        guard components.count >= 3,
              let id = UUID(uuidString: components[1]),
              let index = Int(components[2]) else {
            send(connection, response: makeJSONResponse(["error": "Subtitle not found"], status: "404 Not Found"))
            return
        }

        Task { @MainActor in
            guard let item = self.library.item(id: id), item.subtitlePaths.indices.contains(index) else {
                self.send(connection, response: self.makeJSONResponse(["error": "Subtitle not found"], status: "404 Not Found"))
                return
            }
            let subtitlePath = item.subtitlePaths[index]
            self.queue.async {
                ...
                let contentType = url.pathExtension.lowercased() == "vtt" ? "text/vtt; charset=utf-8" : "application/x-subrip; charset=utf-8"
                self.send(connection, response: self.makeResponse(status: "200 OK", contentType: contentType, body: data))
            }
        }
    }
    ...
    private func handlePlaybackSession(connection: NWConnection, request: HTTPRequest, path: String) {
        ...
        case "POST", "PATCH", "PUT":
            updatePlaybackSession(connection: connection, path: path, body: request.body)
        default:
            send(connection, response: makeJSONResponse(["error": "Method not allowed"], status: "405 Method Not Allowed"))
        }
    }

    private func sendPlaybackSession(connection: NWConnection, path: String) {
        guard let id = SonderHTTPRouteID.uuid(from: path) else {
    ...
    private func updatePlaybackSession(connection: NWConnection, path: String, body: Data) {
        guard let id = SonderHTTPRouteID.uuid(from: path) else {
            send(connection, response: makeJSONResponse(["error": "Invalid item id"], status: "400 Bad Request"))
            return
        }
        ...
        guard let payload = try? decoder.decode(SonderProgressUpdate.self, from: body) else {
            send(connection, response: makeJSONResponse(["error": "Invalid playback payload"], status: "400 Bad Request"))
            return
        }
        Task { @MainActor in
            self.library.updatePlaybackSession(itemID: id, update: payload)
            self.sendPlaybackSession(connection: connection, path: path)
        }
    }

    @MainActor
    private func sidecarSubtitleTracks(for item: SonderMediaItem) -> [SonderPlaybackTrack] {
        item.subtitlePaths.enumerated().map { offset, path in
            let url = URL(fileURLWithPath: path)
            return SonderPlaybackTrack(
                id: "sidecar:\(offset)",
                label: url.deletingPathExtension().lastPathComponent,
                languageCode: nil,
                kind: "sidecar",
                url: "/subtitles/\(item.id.uuidString)/\(offset)"
            )
        }
    }

...
class PlanTest(unittest.TestCase):
    ...
    def test_noisy_folder_name_moves_the_whole_book(self):
        touch(self.root / "Miguel de Cervantes/Don Quixote [Edition] (2020)/Don Quixote [Edition] (2020).m4b")
        f = self.folder(build_plan(self.root), "Miguel de Cervantes/Don Quixote [Edition] (2020)")
    ...
    def test_competing_editions_are_flagged_for_review(self):
        touch(self.root / "Michael Crichton/Jurassic Park/Jurassic Park.m4b", b"a" * 100)
        touch(self.root / "Michael Crichton/Jurassic Park [Unabridged]/Jurassic Park [Unabridged].m4b", b"a" * 200)
        plan = build_plan(self.root)
        reviews = [f for f in plan["folders"] if f["action"] == "review"]
        self.assertEqual(len(reviews), 2)
        self.assertTrue(all(f["group"] == "single_file" for f in reviews))
...
class ApplyTest(unittest.TestCase):
    ...
    def test_apply_moves_book_folder_and_matches_file_name(self):
        touch(self.root / "Author/Book [Edition] (2020)/Book [Edition] (2020).m4b", b"audio")
        plan = build_plan(self.root)
        ...
        self.assertFalse((self.root / "Author/Book [Edition] (2020)").exists())

    def test_apply_refuses_to_overwrite_an_existing_book_folder(self):
        touch(self.root / "Author/Book [Edition]/Book [Edition].m4b", b"new")
        ...
        self.assertTrue((self.root / "Author/Book [Edition]").is_dir())
